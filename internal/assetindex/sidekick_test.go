...
func TestSidekickDefinitionSurvivesCRLFAndBlankLines(t *testing.T) {
	for _, tc := range []struct{ name, sk string }{
		...
		// A comment is valid YAML at any column, and at column zero it read as a
		// top-level key, which closes the block exactly as the blank line above did.
		{"a comment at column zero inside Parts", "Name: Hero\nParts:\n- Name: SK_HEAD\n# generated by the exporter\n- Name: SK_TORS\n"},
		{"an indented comment inside Parts", "Name: Hero\nParts:\n- Name: SK_HEAD\n  # a note\n- Name: SK_TORS\n"},
		{"a comment before the block", "# quarry\nName: Hero\nParts:\n- Name: SK_HEAD\n- Name: SK_TORS\n"},
	} {
		t.Run(tc.name, func(t *testing.T) {
